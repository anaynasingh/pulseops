...
- [2026-07-06] When users can authenticate via multiple credential types, never gate "is this a real user" on a single credential column.
         Builder: claude.
         Why: PulseOps GET /users/ (the task-assignment dropdown source) filtered on password_hash.isnot(None). Microsoft SSO login creates users with ms_oid set and password_hash NULL, so every SSO user was silently excluded from assignment, leaving only the 3 legacy password-seeded accounts. The filter looked correct in isolation and passed typecheck. Source: assignable-users-sso.
         How to apply: when a table stores more than one credential type (password_hash, ms_oid / oauth sub, api_key, etc.), the "has authenticated / is a real user" predicate is the OR across all credential columns, not any single one. At /ag-plan, scan any user-listing / visibility / eligibility query for single-credential filters and require the full OR. Keep an independent domain/active guard for placeholder/service accounts rather than leaning on a credential column to exclude them.

- [2026-07-06] When an input can be one of several credential or type classes and you fall back between handlers, branch on WHAT THE THING IS, not on whether the first handler produced a result.
         Builder: claude.
         Why: the mcp-longlived-apikey plan first specified "if the JWT path did not yield a user, fall back to api_key" — which would let a valid JWT for an inactive/missing user silently attempt an api_key match, muddying the auth contract (a rejected-but-valid JWT is not an api_key). Codex plan-challenge C3 (HIGH) caught it before build. Correct gate: `if decode_token(t) is not None:` handle entirely as a JWT including its own 401s; `else:` try api_key.
         How to apply: any fallback between credential types, request-body defaulting, or cache-miss handling — gate on the identity/type of the input (is it a JWT? is the field in model_fields_set?), never on "did lookup A return empty". Same shape as the omit-vs-null model_fields_set contract (intake-default-assignee). An item valid for class A but rejected must fail in class A's branch, not fall through to class B. Recurs across auth, PATCH/PUT defaulting, and cache handling — global-promotion candidate.
